...
namespace WebApplication1.Controllers
{
    public class AdminController : Controller
    {
        ...
        [ValidateInput(false)]
        public ActionResult ConfirmOnEditEditAdmin(Admin ad, HttpPostedFileBase fileupload, FormCollection collection)
        {
            //var am = from admin in db.Admins where admin.AdminID.ToString() == Session["Username"].ToString() select admin.MaAdmin;
            ...
            //ad = db.Admins.SingleOrDefault(n => n.MaAdmin == am.Single());
            if (String.IsNullOrEmpty(hoten))
            {
                @ViewData["Loi1"] = "Vui lòng cho biết họ tên của bạn";
            }
            else if (String.IsNullOrEmpty(email))
            {
                @ViewData["Loi2"] = "Vui lòng cho biết địa chỉ email của bạn";
            }
            else if (String.IsNullOrEmpty(matkhau))
            {
                @ViewData["Loi3"] = "Vui lòng cho biết mật khẩu bạn đang dùng để đăng nhập";
            }
            else if (String.IsNullOrEmpty(sdt))
            {
                @ViewData["Loi4"] = "Vui lòng cho biết số diện thoại của bạn";
            }
            else if (fileupload == null)
            {
                //ad = db.Admins.SingleOrDefault(n => n.MaAdmin == am.Single());
                ...
                //@ViewData["Loi6"] = "Vui lòng chọn 1 tấm hình";
        ...
        public ActionResult ConfirmOnCreateAdmin(Admin ad, HttpPostedFileBase fileupload, FormCollection collection)
        {
            var hoten = collection["HoTen"];
            var email = collection["Email"];
            var sdt = collection["SDT"];
            ...
            {
                @ViewData["Loi5"] = "Vui lòng cho biết tên đăng nhập";
            }
            else if (String.IsNullOrEmpty(sdt))
            {
                @ViewData["Loi4"] = "Vui lòng cho biết số diện thoại của bạn";
        ...
        public ActionResult Avatar ()
        {
            var av = (from a 
                     in db.Admins 
                     where a.AdminID.ToString() == @Session["Username"].ToString() 
                     select a).ToList();
            return PartialView(av.Single());
        }
    }
}